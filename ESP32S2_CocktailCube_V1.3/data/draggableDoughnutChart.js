...
(function ()
{
  ...
  DraggableDoughnutchart.prototype.Setup = function (setupValues)
  {
    ...
    this.onshifted = setupValues.onshifted;
    ...
    function TouchEnd()
    {
      // Check for dragged segment
      if (doughnutchart.draggedSegment)
      {
        // Save new values for event
        var changedIndex = doughnutchart.draggedSegment.index;
        var changedDistance = doughnutchart.draggedSegment.angleDragDistance;
                
        // Check for angle changes
        if (changedDistance != 0)
        {
          doughnutchart.onshifted(changedIndex, changedDistance);
        }
        
        // Reset dragged segment and draw new values
  ...
  {
    // Avoid setting angles if drag operation is active
    if (doughnutchart.draggedSegment)
    {
      return;
    }
  ...
  // Returns the dragged state
  DraggableDoughnutchart.prototype.IsDragged = function()
  {
    // Check for dragged segment
    return doughnutchart.draggedSegment != null;
  };
  ...
  DraggableDoughnutchart.prototype.MoveAngle = function (index, angle)
  {
    ...
    if (angle != 0)
    {
      doughnutchart.onshifted(index, angle);
    }
  };
  ...
  DraggableDoughnutchart.prototype.defaults =
  {
    ...
    onshifted: function (index, increments) { }
  };
